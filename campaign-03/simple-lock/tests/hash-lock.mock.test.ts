...
const VALID_PREIMAGE = "FidelCoder Campaign 03";

function utf8Hex(value: string) {
  return hexFrom(new TextEncoder().encode(value));
}

function buildHashLockTransaction(
...
function expectScriptResult(verifier: Verifier, expectedCode: number) {
...
describe("hash-lock contract", () => {
  test("accepts the matching UTF-8 preimage", () => {
    const { resource, tx } = buildHashLockTransaction(
      VALID_PREIMAGE,
      VALID_PREIMAGE,
    );
    expectScriptResult(Verifier.from(resource, tx), 0);
  });

  test("rejects an incorrect preimage with error code 11", () => {
    const { resource, tx } = buildHashLockTransaction(
      VALID_PREIMAGE,
      "not-the-preimage",
    );
    expectScriptResult(Verifier.from(resource, tx), 11);
  });

  test("rejects an empty witness lock with error code 10", () => {
    const { resource, tx } = buildHashLockTransaction(VALID_PREIMAGE, "");
    expectScriptResult(Verifier.from(resource, tx), 10);
  });

  test("rejects missing hash args with error code 12", () => {
    const { resource, tx, mainScript, contractScript } =
      buildHashLockTransaction(VALID_PREIMAGE, VALID_PREIMAGE);
    mainScript.args = hexFrom(
      "0x0000" +
        contractScript.codeHash.slice(2) +
        hexFrom(hashTypeToBytes(contractScript.hashType)).slice(2),
    );
    expectScriptResult(Verifier.from(resource, tx), 12);
  });
});
